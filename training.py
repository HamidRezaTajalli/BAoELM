...
def trainer(exp_num: int, saving_path: pathlib.Path, elm_type: str, dataset: str, trigger_type: str, hdlyr_size: int or list) -> None:
    ...
    # TODO: doon't forget to gc.collect!!
        ...
        acc_train, acc_test, final_standard_div, (W_list, Beta_list, W_prime_list), elapsed_time = DRELM_main.DRELM_main(all_data['train']['x'],
        ...
        acc_test, acc_train, (Wie, Whe, Beta_new), elapsed_time = TELM_Main.TELM_main(all_data['train']['x'],
                                                  all_data['train']['y_oh'].numpy(),
                                                  all_data['test']['x'],
                                                  all_data['test']['y_oh'].numpy(),
                                                  hidden_size=hdlyr_size)
        ...
        acc_test, elapsed_time = ML_ELM_main.main_ML_ELM(all_data['train']['x'],
                                           all_data['train']['y_oh'].numpy(),
                                           all_data['test']['x'],
                                           all_data['test']['y_oh'].numpy(),
        ...
        acc_test = main_CNNELM.test(model, dataloaders['test']).item()
        print(acc_test)
