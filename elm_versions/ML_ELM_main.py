...
def main_ML_ELM_with_mask(X_train, Y_train, X_test, Y_test, prune_rate, hidden_layer: int = 700):
    ...
    for i in range(1):
        start_time = time.time()
        betahat_1, betahat_2, betahat_3, betahat_4, Y, prune_mask = ML_ELM_train_with_mask(X_train, Y_train, n_hid, 10, CC, prune_rate)
        elapsed_time = time.time() - start_time
